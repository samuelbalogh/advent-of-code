...
def get_sleeping_distribution(data):
    ...
    for guard in snoozie:
        snooziest_minute_for_guard = max(
            snoozie[guard], key=lambda minute: snoozie[guard][minute]
        )
        snooziest_minutes.append(
            (
                guard,
                snooziest_minute_for_guard,
                snoozie[guard][snooziest_minute_for_guard],
            )
        )
